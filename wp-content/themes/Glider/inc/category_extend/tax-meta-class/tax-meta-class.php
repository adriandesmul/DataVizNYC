  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme'])){
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = get_template_directory_uri() . '/inc/category_extend/tax-meta-class';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_text( $field, $meta) {  
    $this->show_field_begin( $field, $meta );
    echo "<input type='text' class='iconname at-text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' style='{$field['style']}' size='30' />";
  ...
  public function addText($id,$args,$repeater=false){
    ...
    public function addIcon($id,$args,$repeater=false){
        $new_field = array('type' => 'text','id'=> $id,'std' => '','desc' => '','style' =>'','name' => 'Icon name Field','multiple' => false);
        $new_field = array_merge($new_field, $args);
        if(false === $repeater){
            $this->_fields[] = $new_field;
        }else{
