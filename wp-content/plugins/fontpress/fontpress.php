...
Description: Personalize your website with custom fonts. Choose if use Cufons, Google and Adobe web fonts or Font-Face fonts. Add and manage unlimited rules to customize whatever you want wherever you want in your pages. Use the shortcode wizard to customize text without knowing CSS selectors.  
...
function fp_global_scripts() { 
	...
	if (is_admin()) {  
		...
		// iphone checks
		wp_enqueue_style( 'lcwp-ip-checks', FP_URL.'/js/iphone_checkbox/style.css', 999);
	...
	if (!is_admin()) {
		if(!get_option('fp_inline_code')) {
			...
			if (file_exists(FP_DIR.'/custom_files/frontend'.FP_BID.'.css')) {
				wp_enqueue_style('fp_frontend_css', FP_URL.'/custom_files/frontend'.FP_BID.'.css', '2.33');		
			}
			
			// frontend js
			if (file_exists(FP_DIR.'/custom_files/frontend'.FP_BID.'.js')) {
				wp_enqueue_script('fp_frontend_js', FP_URL.'/custom_files/frontend'.FP_BID.'.js', '2.33');		
			}
		}
...
function fp_admin_actions() {  
	...
	add_menu_page('FontPress', 'FontPress', $capability, 'fp_settings', 'fp_elements_rules', $menu_img);
	
	// submenus
	add_submenu_page('fp_settings', __('Elements Rules', 'fp_ml'), __('Elements Rules', 'fp_ml'), $capability, 'fp_settings', 'fp_elements_rules');	
	...
	add_submenu_page('fp_settings', __('Manage Font-Face Fonts', 'fp_ml'), __('Manage Font-Face Fonts', 'fp_ml'), $capability, 'fp_man_fontface', 'fp_man_fontface');
}  
add_action('admin_menu', 'fp_admin_actions');  


// elements rules
function fp_elements_rules() {
	include_once(FP_DIR . '/settings.php');
}

// manage cufon page
function fp_man_cufon() {
	include_once(FP_DIR . '/cufons.php');	
}

//  manage google and adobe web fonts page
function fp_man_gwf() {
	include_once(FP_DIR . '/web_fonts.php');	
}

//  manage @font-face fonts
function fp_man_fontface() {
	include_once(FP_DIR . '/fontface.php');	
}
...
$lcun = new lc_update_notifier(__FILE__, 'http://projects.lcweb.it/envato_update/fp.php');
////////////
