...
class simple_fv {
	...
	private function validate($type, $val, $index_val, $label, $test=false) {
		...
		if($type == 'min_val') {
			if((float)$index_val < (int)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = 'minimum value is '.$val;
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'max_val') {
			if((float)$index_val > (int)$val) {
				if(!$test) {
					#$this->errors['value'][] = $label;
					$this->custom_error[$label] = 'maximum value is '.$val;
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'min_len') {
			if(strlen($index_val) < (int)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = 'minimum '.$val.' characters allowed';
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'max_len') {
			if(strlen($index_val) > (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = 'maximum '.$val.' characters allowed';
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'right_len') {
			if(strlen($index_val) != (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = 'has to be long '.$val.' characters';
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'min_array') {
			if(!is_array($index_val) || count($index_val) < $val) {
				if(!$test) {
					...
					$this->custom_error[$label] = 'Select '.$val.' options';
				}
				return false;	
			}
		}
		...
		if($type == 'max_array' && is_array($index_val)) {
			if(count($index_val) > $val) {
				if(!$test) {
					#$this->errors['array_count'][] = $label;
					$this->custom_error[$label] = 'Maximum '.$val.' options allowed';
				}
				return false;	
			}
		}
	...
	private function errorTranslate($type, $label_array) {
		...
		switch($type) {
			case 'required' : 
				(count($label_array) > 1) ? $message .= 'are required' : $message .= 'is required';
				break;
			
			case 'wrong_int' : 
				(count($label_array) > 1) ? $message .= 'are not valid integers' : $message .= 'is not a valid integer';
				break;
				
			case 'wrong_float' : 
				(count($label_array) > 1) ? $message .= 'are not valid floating numbers' : $message .= 'is not a valid floating number';
				break;	
				
			case 'wrong_mail' : 
				(count($label_array) > 1) ? $message .= 'are not valid e-mail addresses' : $message .= 'is not a valid e-mail address';
				break;		
				
			case 'wrong_date' : 
				(count($label_array) > 1) ? $message .= 'are not valid dates' : $message .= 'is not a valid date';
				break;		
				
			case 'wrong_time' : 
				(count($label_array) > 1) ? $message .= 'are not valid times' : $message .= 'is not a valid time';
				break;
				
			case 'wrong_url' : 
				(count($label_array) > 1) ? $message .= 'are not valid urls' : $message .= 'is not a valid url';
				break;	
				
			case 'wrong_hex' : 
				(count($label_array) > 1) ? $message .= 'are not valid hexadecimal colors' : $message .= 'is not a valid hexadecimal color';
				break;	
			
			case 'wrong_ip' : 
				(count($label_array) > 1) ? $message .= 'are not valid IP addresses' : $message .= 'is not a valid IP address';
				break;	
				
			case 'wrong_zip' : 
				(count($label_array) > 1) ? $message .= 'are not valid ZIP codes' : $message .= 'is not a valid ZIP code';
				break;
				
			case 'wrong_tel' : 
				(count($label_array) > 1) ? $message .= 'are not valid telephone numbers' : $message .= 'is not a valid telephone number';
				break;		
					
			case 'wrong_type' : $message .= 'invalid data inserted';
				break;	
				
			case 'allowed' : 
				(count($label_array) > 1) ? $message .= 'values are not between the allowed' : $message .= 'value is not between the allowed';
				break;
				
			case 'forbidden' : 
				(count($label_array) > 1) ? $message .= 'values are between the forbidden' : $message .= 'values is between the forbidden';
				break;
				
			case 'not_equal' : $message .= "the value doesn't match"; 
				break;
				
			case 'size' : $message .= 'file size is wrong';
				break;						
		}	
