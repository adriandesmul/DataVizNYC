...
class cmb_Meta_Box_types {
	...
	public static function sidebar_select( $field, $meta ) {
		$meta = self::esc( $meta );
		...
		if (class_exists('SMK_Sidebar_Generator')){
			...
			if ( function_exists('smk_get_all_sidebars') ) {
				...
				_e('Please install/update SMK Sidebar Generator plugin in Apperance -> Install plugins','crum');
			}
		} else {
			_e('Please install SMK Sidebar Generator plugin in Apperance -> Install plugins','crum');
		}
	}
